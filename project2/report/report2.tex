...
When generating our hash-function we create a list of 80 random numbers in the range $0;p$ since we know that a line of text can't be longer than 80 characters. Then when hashing a line we find $X = MAATH-med-sum-her$. Each time we compare two given files/streams we pick a new $z$ to calculate the value of the evaluated pylonomial with.

In our tests we show that we can stream the test-data, but all of our tests are run with the test-data in memory, due to the overhead involved with reading files. The error-prone part of the algorithm has nothing to do with how we read the file, streaming or from memory, so that is not a problem.
