...
Let $X$ denote the number of comparisons made by randomized \texttt{QUICKSORT} on an input of length $n$. We know that $E[X] \leq \mu_s = 2n\ln n$. Prove that the probability that the number of comparisons made in an execcution of randomized \texttt{QUICKSORT} exceeds $3\mu_s$ is at most $2n^{-d}$ for as large a constant $d$ as you can. 

\todo{Forklar den egentlige ulighed og hvorfor den gælder}

\begin{align*}
n\cdot \Pr[Z>3\mu] &\leq n\cdot \left [ \frac{e^2}{3^2} \right ]^{2\ln n}\\
=n\cdot\frac{e^{4\ln n}}{9^{2\ln n}} &=\frac{n^5}{9^{2\ln n}} =\frac{n^5}{e^{\ln 9 \ln n^2}}\\
=\frac{n^5}{n^{2\ln 9}} &=\frac{n^5}{n^{\ln 9}n^{\ln 9}} =n^{5-2\ln 9}
\end{align*}
\todo{Pænere alignment}
