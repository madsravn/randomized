...
\textbf{Gør det lige tydeligt hvor vi svarer opgave 1.1 og 1.2}

\subsection{Subproblem 1.3} 
Prove that $E[Z]\leq \mu = 2\ln n$.\\
For any $k, 0<k<n$ we have that $Z = \sum_{i=1}^{n} \frac{1}{|k-i|+1}$.
This gives cause to the following: \textbf{Jeg tror ikke vi kan bruge ilig tegn mellem summe og harmoniske numre}
...
XS$$\Pr \left ( X > (1+\delta)\mu \right ) < \left [ \frac{e^\delta}{(1+\delta)^{1+\delta}} \right ]^\mu$$
...
\textbf{For each n, k pair you should run a large number of experiments (at least, pref more).}
\textbf{Når vi når hertil, så formater lige opgavestillingen lidt bedre}
...
=n\cdot\frac{e^{4\ln n}}{9^{2\ln n}}\\
=\frac{n^5}{9^{2\ln n}}\\
=\frac{n^5}{e^{\ln 9 \ln n^2}}\\
=\frac{n^5}{n^{2\ln 9}}\\
=\frac{n^5}{n^{\ln 9}n^{\ln 9}}\\
=n^{5-2\ln 9}$
